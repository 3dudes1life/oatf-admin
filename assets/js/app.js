(() => {
  ...
  document.addEventListener('click',event=>{
    ...
    const action=event.target.closest('[data-action]')?.dataset.action;if(action==='export'){const date=todayISO();Store.state.preferences.lastBackup=Store.nowISO();Store.save({immediate:true});UI.download(`oatf-os-production-backup-${date}.json`,Store.exportData());UI.toast('Backup exported','Production data downloaded.');closePopovers();return;}if(action==='import'){$('#importInput').click();closePopovers();return;}
  });
