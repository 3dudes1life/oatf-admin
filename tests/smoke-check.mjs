...
if (!html.includes('assets/js/release-center.js')) failures.push('Release Center script is not loaded.');

const store = fs.readFileSync(path.join(root, 'assets/js/store.js'), 'utf8');
if (!store.includes("oatf-os-production-v010")) failures.push('V0.10 storage key not found.');
if (!store.includes("version:'0.10'")) failures.push('V0.10 state version not found.');

const serviceWorker = fs.readFileSync(path.join(root, 'sw.js'), 'utf8');
if (!serviceWorker.includes('oatf-os-production-v010')) failures.push('V0.10 cache name not found.');
if (!serviceWorker.includes('release-center.js')) failures.push('Release Center missing from offline cache.');

const manifest = JSON.parse(fs.readFileSync(path.join(root, 'manifest.webmanifest'), 'utf8'));
if (manifest.version !== '0.10') failures.push('Manifest version is not 0.10.');

if (failures.length) {
  console.error('OATF OS V0.10 smoke check FAILED');
  failures.forEach(failure => console.error(` - ${failure}`));
  process.exit(1);
}

console.log('OATF OS V0.10 smoke check PASSED');
